/*
 * role-overlay-starrocks-vault-agents.tf -- Phase 0.G.6
 *
 * Installs nexus-vault-agent on each of the 6 StarRocks nodes (3 FE + 3 BE).
 * Port of the ClickHouse vault-agents overlay with StarRocks specifics:
 * sidecar filename vault-agent-analytics-starrocks-<host>.json + the SR node set.
 *
 * Cross-env coupling: reads the AppRole creds JSON sidecars (security env writes
 * them). ERROR (not WARN+skip) if absent.
 *
 * Selective ops: var.enable_starrocks_vault_agents (master) AND per-host
 *                var.enable_<host>_vault_agent.
 */

locals {
  starrocks_vault_agent_specs = {
    "sr-fe-leader"     = { vm_ip = "192.168.70.31", enabled = var.enable_sr_fe_leader_vault_agent }
    "sr-fe-follower-1" = { vm_ip = "192.168.70.32", enabled = var.enable_sr_fe_follower_1_vault_agent }
    "sr-fe-follower-2" = { vm_ip = "192.168.70.33", enabled = var.enable_sr_fe_follower_2_vault_agent }
    "sr-be-1"          = { vm_ip = "192.168.70.34", enabled = var.enable_sr_be_1_vault_agent }
    "sr-be-2"          = { vm_ip = "192.168.70.35", enabled = var.enable_sr_be_2_vault_agent }
    "sr-be-3"          = { vm_ip = "192.168.70.36", enabled = var.enable_sr_be_3_vault_agent }
  }

  starrocks_vault_agent_active = {
    for host, spec in local.starrocks_vault_agent_specs : host => spec
    if var.enable_starrocks_vault_agents && spec.enabled
  }

  sr_va_creds_dir_expanded = pathexpand(replace(var.vault_agent_starrocks_creds_dir, "$HOME", "~"))
  sr_va_ca_bundle_expanded = pathexpand(replace(var.vault_pki_ca_bundle_path, "$HOME", "~"))
}

resource "null_resource" "starrocks_vault_agent" {
  for_each = local.starrocks_vault_agent_active

  triggers = {
    creds_file_path = "${local.sr_va_creds_dir_expanded}/vault-agent-analytics-starrocks-${each.key}.json"
    creds_file_hash = filesha256("${local.sr_va_creds_dir_expanded}/vault-agent-analytics-starrocks-${each.key}.json")
    nftables_id     = length(null_resource.starrocks_nftables_backplane) > 0 ? null_resource.starrocks_nftables_backplane[0].id : "disabled"
  ...
  depends_on = [null_resource.starrocks_nftables_backplane]
  ...
  provisioner "local-exec" {
    ...
    command     = <<-PWSH
      ...
      $credsFile    = '${local.sr_va_creds_dir_expanded}/vault-agent-analytics-starrocks-${each.key}.json'
      $caBundlePath = '${local.sr_va_ca_bundle_expanded}'
      $sshUser      = '${var.analytics_node_user}'
      $bootTimeout  = ${var.analytics_cluster_timeout_minutes}

      if (-not (Test-Path $credsFile)) {
        throw "[sr-va $hostName] creds file $credsFile missing -- run nexus-infra-vmware/scripts/security.ps1 apply FIRST to provision the 6 StarRocks AppRole sidecars."
      }
      $creds     = Get-Content $credsFile | ConvertFrom-Json
      $roleId    = $creds.role_id
      $secretId  = $creds.secret_id
      $vaultAddr = $creds.vault_addr
      if (-not $roleId -or -not $secretId) { throw "[sr-va $hostName] creds JSON missing role_id or secret_id" }
      if (-not (Test-Path $caBundlePath)) { throw "[sr-va $hostName] CA bundle $caBundlePath missing -- run security env apply first." }

      $sshOpts = @('-o','ConnectTimeout=10','-o','BatchMode=yes','-o','StrictHostKeyChecking=no')

      Write-Host "[sr-va $hostName] waiting for SSH + firstboot marker..."
      ...
      if (-not $booted) { throw "[sr-va $hostName] SSH + firstboot marker never ready after $bootTimeout min" }
      ...
      if ($probe -match "Vault v$vaultVersion" -and $probe -match '(?m)^active$') {
        Write-Host "[sr-va $hostName] already installed at v$vaultVersion + service active; skipping."
        exit 0
      }

      Write-Host "[sr-va $hostName] installing Vault Agent v$vaultVersion"
      $installScript = @"
set -euo pipefail
...
"@
      ...
      if ($LASTEXITCODE -ne 0) { Write-Host $installOut.Trim(); throw "[sr-va $hostName] vault binary install failed (rc=$LASTEXITCODE)" }
...
"@
        ...
        if ($LASTEXITCODE -ne 0) { Write-Host $stageOut.Trim(); throw "[sr-va $hostName] credential staging failed (rc=$LASTEXITCODE)" }
      } finally {
      ...
      $baseConfig = @"
# 00-base.hcl -- Phase 0.G.6 (StarRocks mTLS).
...
"@
      $unitFile = @"
[Unit]
Description=Nexus Vault Agent (Phase 0.G.6 -- StarRocks mTLS)
...
"@
      ...
      if ($LASTEXITCODE -ne 0) { Write-Host $finalOut.Trim(); throw "[sr-va $hostName] config/service setup failed (rc=$LASTEXITCODE)" }
      ...
      if (-not $serviceActive) {
        ...
        Write-Host $journal
        throw "[sr-va $hostName] nexus-vault-agent.service failed to reach active within 30s"
      }
      $tokenCheck = (ssh @sshOpts "$sshUser@$vmIp" "sudo test -s /var/run/nexus-vault-agent/token && echo TOKEN_PRESENT" 2>&1 | Out-String).Trim()
      if ($tokenCheck -notmatch 'TOKEN_PRESENT') { throw "[sr-va $hostName] AppRole login appears to have failed (token sink empty)" }
      Write-Host "[sr-va $hostName] nexus-vault-agent.service active; AppRole authenticated"
    PWSH
  }
  ...
  provisioner "local-exec" {
    ...
    command     = <<-PWSH
      ...
      Write-Host "[sr-va destroy] $${hostName}: stopping nexus-vault-agent + cleaning install-owned files"
