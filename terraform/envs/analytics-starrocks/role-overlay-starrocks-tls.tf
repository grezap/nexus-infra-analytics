/*
 * role-overlay-starrocks-tls.tf -- Phase 0.G.6 -- StarRocks mTLS cert render
 *
 ...
 * starrocks-fe.nexus.lab in their SANs (ADR-0031).
 *
 ...
 *
 * Selective ops: var.enable_starrocks_tls AND var.enable_starrocks_vault_agents.
 */

locals {
  starrocks_tls_per_host = {
    "sr-fe-leader"     = { vmnet10 = "192.168.10.31", vmnet11 = "192.168.70.31", role = "fe", dest_dir = "/opt/starrocks/fe/conf/tls" }
    "sr-fe-follower-1" = { vmnet10 = "192.168.10.32", vmnet11 = "192.168.70.32", role = "fe", dest_dir = "/opt/starrocks/fe/conf/tls" }
    "sr-fe-follower-2" = { vmnet10 = "192.168.10.33", vmnet11 = "192.168.70.33", role = "fe", dest_dir = "/opt/starrocks/fe/conf/tls" }
    "sr-be-1"          = { vmnet10 = "192.168.10.34", vmnet11 = "192.168.70.34", role = "be", dest_dir = "/opt/starrocks/be/conf/tls" }
    "sr-be-2"          = { vmnet10 = "192.168.10.35", vmnet11 = "192.168.70.35", role = "be", dest_dir = "/opt/starrocks/be/conf/tls" }
    "sr-be-3"          = { vmnet10 = "192.168.10.36", vmnet11 = "192.168.70.36", role = "be", dest_dir = "/opt/starrocks/be/conf/tls" }
  }

  starrocks_tls_active = {
    for host, spec in local.starrocks_tls_per_host : host => spec
    if(
      var.enable_starrocks_tls && var.enable_starrocks_vault_agents
      && lookup(local.starrocks_vault_agent_active, host, null) != null
    )
  }
}

resource "null_resource" "starrocks_tls" {
  for_each = local.starrocks_tls_active

  triggers = {
    va_id           = null_resource.starrocks_vault_agent[each.key].id
    pki_role_name   = var.vault_pki_starrocks_role_name
  ...
  depends_on = [null_resource.starrocks_vault_agent]
  ...
  provisioner "local-exec" {
    ...
    command     = <<-PWSH
      ...
      $pkiRole  = '${var.vault_pki_starrocks_role_name}'
      $sshUser  = '${var.analytics_node_user}'
      $cn       = "$hostName.starrocks.nexus.lab"
      $altNames = "$hostName,$hostName.nexus.lab,$hostName.starrocks.nexus.lab,starrocks-fe.nexus.lab,localhost"
      $ipSans   = "$vmnet10,$ip,127.0.0.1"
      $sshOpts  = @('-o','ConnectTimeout=10','-o','BatchMode=yes','-o','StrictHostKeyChecking=no')

      Write-Host "[sr-tls $hostName] cert render via Vault Agent PKI template -> $destDir"

      $splitScript = @'
#!/bin/bash
set -euo pipefail
DEST="$${1:?usage: starrocks-tls-split.sh DEST_DIR GROUP}"
GROUP="$${2:?usage: starrocks-tls-split.sh DEST_DIR GROUP}"
...
install -m 0644 -o root -g root "$TMP/ca-chain.pem" /etc/ssl/certs/starrocks-ca.pem
...
'@
      ...
      $vaultAgentTemplate = @"
# 60-template-starrocks-tls.hcl -- Phase 0.G.6 (rendered for $hostName).
template {
  contents = <<EOT
...
EOT
  ...
  user            = "root"
  group           = "starrocks"
  command         = "/usr/local/sbin/starrocks-tls-split.sh $destDir starrocks"
  command_timeout = "30s"
}
"@
...
echo '$splitB64' | base64 -d | sudo tee /usr/local/sbin/starrocks-tls-split.sh > /dev/null
sudo chown root:root /usr/local/sbin/starrocks-tls-split.sh
sudo chmod 0755 /usr/local/sbin/starrocks-tls-split.sh
echo '$vaB64' | base64 -d | sudo tee /etc/vault-agent/60-template-starrocks-tls.hcl > /dev/null
sudo chown root:root /etc/vault-agent/60-template-starrocks-tls.hcl
sudo chmod 0644 /etc/vault-agent/60-template-starrocks-tls.hcl
...
if ! sudo test -s "$destDir/bundle.pem"; then
  echo "[sr-tls stage] ERROR: bundle.pem not rendered within 20s" >&2
  sudo journalctl -u nexus-vault-agent.service --no-pager -n 20 >&2
  exit 1
fi
sudo /usr/local/sbin/starrocks-tls-split.sh "$destDir" starrocks
echo STAGE_OK
"@
      $stageOut = ($stage -replace "`r`n","`n") | ssh @sshOpts "$sshUser@$ip" "tr -d '\r' | bash -s" 2>&1 | Out-String
      if ($LASTEXITCODE -ne 0 -or $stageOut -notmatch 'STAGE_OK') { Write-Host $stageOut.Trim(); throw "[sr-tls $hostName] cert render stage failed (rc=$LASTEXITCODE)" }
      ...
      if (-not $rendered) { throw "[sr-tls $hostName] cert files not rendered (CN=$cn) within 60s" }
      Write-Host "[sr-tls $hostName] cert rendered (CN=$cn) in $destDir"
    PWSH
  }
  ...
  provisioner "local-exec" {
    ...
    command     = <<-PWSH
      ...
      ssh @sshOpts "$sshUser@$vmIp" "sudo rm -f /etc/vault-agent/60-template-starrocks-tls.hcl $destDir/bundle.pem $destDir/server.crt $destDir/server.key $destDir/ca.crt /etc/ssl/certs/starrocks-ca.pem; sudo systemctl restart nexus-vault-agent.service 2>/dev/null" 2>$null
      exit 0
    PWSH
  }
}
