/*
 * role-overlay-clickhouse-vault-agents.tf -- Phase 0.G.5
 *
 * Installs Vault Agent as a `nexus-vault-agent` systemd service on each of the
 * 9 ClickHouse nodes (3 Keeper + 6 data). Port of nexus-infra-oltp's
 * role-overlay-redis-vault-agents.tf with analytics specifics:
 *   - sidecar filename `vault-agent-analytics-clickhouse-<host>.json`
 *     (produced by nexus-infra-vmware security env's clickhouse AppRole overlay)
 *   - firstboot marker `/var/lib/analytics-node-firstboot-done`
 *
 * Each agent authenticates to Vault via its narrow AppRole and writes a token
 * sink; role-overlay-clickhouse-tls.tf drops a PKI template stanza into the
 * same /etc/vault-agent/ dir without rewriting the base config.
 *
 * Cross-env coupling: reads the AppRole creds JSON sidecars. ERROR (not
 * WARN+skip) if absent. Operator order: security.ps1 apply FIRST.
 *
 * Selective ops: var.enable_clickhouse_vault_agents (master) AND per-host
 *                var.enable_<host>_vault_agent.
 */

locals {
  clickhouse_vault_agent_specs = {
    "ch-keeper-1"    = { vm_ip = "192.168.70.41", enabled = var.enable_ch_keeper_1_vault_agent }
    "ch-keeper-2"    = { vm_ip = "192.168.70.42", enabled = var.enable_ch_keeper_2_vault_agent }
    "ch-keeper-3"    = { vm_ip = "192.168.70.43", enabled = var.enable_ch_keeper_3_vault_agent }
    "ch-shard1-rep1" = { vm_ip = "192.168.70.44", enabled = var.enable_ch_shard1_rep1_vault_agent }
    "ch-shard1-rep2" = { vm_ip = "192.168.70.45", enabled = var.enable_ch_shard1_rep2_vault_agent }
    "ch-shard2-rep1" = { vm_ip = "192.168.70.46", enabled = var.enable_ch_shard2_rep1_vault_agent }
    "ch-shard2-rep2" = { vm_ip = "192.168.70.47", enabled = var.enable_ch_shard2_rep2_vault_agent }
    "ch-shard3-rep1" = { vm_ip = "192.168.70.48", enabled = var.enable_ch_shard3_rep1_vault_agent }
    "ch-shard3-rep2" = { vm_ip = "192.168.70.49", enabled = var.enable_ch_shard3_rep2_vault_agent }
  }

  clickhouse_vault_agent_active = {
    for host, spec in local.clickhouse_vault_agent_specs : host => spec
    if var.enable_clickhouse_vault_agents && spec.enabled
  }

  # pathexpand() handles `~` only, not `$HOME`; substitute first.
  ch_va_creds_dir_expanded = pathexpand(replace(var.vault_agent_clickhouse_creds_dir, "$HOME", "~"))
  ch_va_ca_bundle_expanded = pathexpand(replace(var.vault_pki_ca_bundle_path, "$HOME", "~"))
}

resource "null_resource" "clickhouse_vault_agent" {
  for_each = local.clickhouse_vault_agent_active

  triggers = {
    creds_file_path = "${local.ch_va_creds_dir_expanded}/vault-agent-analytics-clickhouse-${each.key}.json"
    creds_file_hash = filesha256("${local.ch_va_creds_dir_expanded}/vault-agent-analytics-clickhouse-${each.key}.json")
    nftables_id     = length(null_resource.clickhouse_nftables_backplane) > 0 ? null_resource.clickhouse_nftables_backplane[0].id : "disabled"
  ...
  depends_on = [null_resource.clickhouse_nftables_backplane]
  ...
  provisioner "local-exec" {
    ...
    command     = <<-PWSH
      ...
      $credsFile    = '${local.ch_va_creds_dir_expanded}/vault-agent-analytics-clickhouse-${each.key}.json'
      $caBundlePath = '${local.ch_va_ca_bundle_expanded}'
      $sshUser      = '${var.analytics_node_user}'
      $bootTimeout  = ${var.analytics_cluster_timeout_minutes}

      if (-not (Test-Path $credsFile)) {
        throw "[ch-va $hostName] creds file $credsFile missing -- run nexus-infra-vmware/scripts/security.ps1 apply FIRST to provision the 9 ClickHouse AppRole sidecars."
      }
      $creds     = Get-Content $credsFile | ConvertFrom-Json
      $roleId    = $creds.role_id
      $secretId  = $creds.secret_id
      $vaultAddr = $creds.vault_addr
      if (-not $roleId -or -not $secretId) { throw "[ch-va $hostName] creds JSON missing role_id or secret_id" }
      if (-not (Test-Path $caBundlePath)) { throw "[ch-va $hostName] CA bundle $caBundlePath missing -- run security env apply (PKI distribute) first." }

      $sshOpts = @('-o','ConnectTimeout=10','-o','BatchMode=yes','-o','StrictHostKeyChecking=no')

      Write-Host "[ch-va $hostName] waiting for SSH + firstboot marker..."
      ...
      if (-not $booted) { throw "[ch-va $hostName] SSH + firstboot marker never ready after $bootTimeout min" }
      ...
      if ($probe -match "Vault v$vaultVersion" -and $probe -match '(?m)^active$') {
        Write-Host "[ch-va $hostName] already installed at v$vaultVersion + service active; skipping."
        exit 0
      }

      Write-Host "[ch-va $hostName] installing Vault Agent v$vaultVersion"

      $installScript = @"
set -euo pipefail
# Ensure DNS works before any outbound HTTP (deb13 baseline can land with an
# empty /etc/resolv.conf on fresh clones -- feedback_deb13_baseline_dns_resolver.md).
if ! getent hosts releases.hashicorp.com >/dev/null 2>&1; then
  echo "[ch-va install] pointing /etc/resolv.conf at nexus-gateway dnsmasq"
...
"@
      ...
      if ($LASTEXITCODE -ne 0) { Write-Host $installOut.Trim(); throw "[ch-va $hostName] vault binary install failed (rc=$LASTEXITCODE)" }
...
"@
        ...
        if ($LASTEXITCODE -ne 0) { Write-Host $stageOut.Trim(); throw "[ch-va $hostName] credential staging failed (rc=$LASTEXITCODE)" }
      } finally {
      ...
      $baseConfig = @"
# 00-base.hcl -- Phase 0.G.5. auto_auth (approle) + sink + vault address.
# role-overlay-clickhouse-tls.tf drops 60-template-clickhouse-tls.hcl alongside.
...
"@
      $unitFile = @"
[Unit]
Description=Nexus Vault Agent (Phase 0.G.5 -- ClickHouse mTLS)
...
"@
      ...
      if ($LASTEXITCODE -ne 0) { Write-Host $finalOut.Trim(); throw "[ch-va $hostName] config/service setup failed (rc=$LASTEXITCODE)" }
      ...
      if (-not $serviceActive) {
        ...
        Write-Host $journal
        throw "[ch-va $hostName] nexus-vault-agent.service failed to reach active within 30s"
      }
      Write-Host "[ch-va $hostName] nexus-vault-agent.service active"
      ...
      if ($tokenCheck -notmatch 'TOKEN_PRESENT') {
        $journal = (ssh @sshOpts "$sshUser@$vmIp" "sudo journalctl -u nexus-vault-agent.service --no-pager -n 30" 2>&1 | Out-String)
        Write-Host $journal
        throw "[ch-va $hostName] AppRole login appears to have failed (token sink empty)"
      }
      Write-Host "[ch-va $hostName] AppRole authenticated; token sink populated"
    PWSH
  }

  # Surgical destroy -- removes only the files THIS overlay installs (keeps
  # /etc/vault-agent/ + the TLS template overlay drops there).
  provisioner "local-exec" {
    when        = destroy
    interpreter = ["pwsh", "-NoProfile", "-Command"]
    command     = <<-PWSH
      ...
      Write-Host "[ch-va destroy] $${hostName}: stopping nexus-vault-agent + cleaning install-owned files"
