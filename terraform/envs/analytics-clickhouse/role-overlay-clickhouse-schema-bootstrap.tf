...
resource "null_resource" "clickhouse_schema_bootstrap" {
  ...
  provisioner "local-exec" {
    ...
    command     = <<-PWSH
      ...
      $bootTmpl = @'
set -euo pipefail
CH() { clickhouse-client --secure --host localhost --port 9440 "$@"; }
...
CH --query "CREATE TABLE IF NOT EXISTS nexus.events_local ON CLUSTER __CLUSTER__ (event_id UInt64, ts DateTime, bucket UInt32, payload String) ENGINE = ReplicatedMergeTree('/clickhouse/tables/{shard}/nexus/events_local', '{replica}') ORDER BY (ts, event_id)"
CH --query "CREATE TABLE IF NOT EXISTS nexus.events ON CLUSTER __CLUSTER__ AS nexus.events_local ENGINE = Distributed(__CLUSTER__, nexus, events_local, rand())"
echo "[schema-bootstrap] schema created (events_local ReplicatedMergeTree + events Distributed)"
...
# 3b. Distributed INSERT fans across all 3 shards.
...
'@
      ...
      foreach ($s in $shards) {
        ...
        while ((Get-Date) -lt $deadline) {
          $c1 = [int](ssh @sshOpts "$sshUser@$($s.rep1)" "clickhouse-client --secure --host localhost --port 9440 --query 'SELECT count() FROM nexus.events_local' 2>/dev/null" 2>&1 | Out-String).Trim()
          $c2 = [int](ssh @sshOpts "$sshUser@$($s.rep2)" "clickhouse-client --secure --host localhost --port 9440 --query 'SELECT count() FROM nexus.events_local' 2>/dev/null" 2>&1 | Out-String).Trim()
  ...
  provisioner "local-exec" {
    ...
    command     = <<-PWSH
      ...
      $drop = "clickhouse-client --secure --host localhost --port 9440 --query 'DROP DATABASE IF EXISTS nexus ON CLUSTER $cn SYNC' 2>/dev/null; clickhouse-client --secure --host localhost --port 9440 --query 'DROP USER IF EXISTS app, admin ON CLUSTER $cn' 2>/dev/null; clickhouse-client --secure --host localhost --port 9440 --query 'DROP ROLE IF EXISTS app_ro, app_rw ON CLUSTER $cn' 2>/dev/null"
