/*
 * analytics-starrocks-sd-fe-node -- Packer template variables (Phase 0.L.5, ADR-0037)
 *
 * Per-engine template: installs a JDK + the StarRocks FE (Frontend) from the
 * StarRocks release tarball. The CN ships in the sibling analytics-starrocks-
 * sd-cn-node template. Separate from the sealed analytics-starrocks-fe-node
 * (0.G.6) per the per-engine + per-cluster-state canon.
 */

variable "vm_name" {
  type        = string
  default     = "analytics-starrocks-sd-fe-node"
  description = "VM display name + output .vmx basename. Per-clone names (sr-sd-fe-1/2/3) set by terraform/envs/analytics-starrocks-sd/."
}

variable "output_directory" {
  type        = string
  default     = "H:/VMS/NexusPlatform/_templates/analytics-starrocks-sd-fe-node"
...
variable "cpus" {
  type        = number
  default     = 2
  description = "Build-time vCPU (matches the sr-sd-fe spec, vms.yaml: 2 vCPU)."
}

variable "memory_mb" {
  type        = number
  default     = 3072
  description = "Build-time RAM (MB). Default 3 GB per ADR-0037 canon (FE is metadata-only in shared-data mode; no tablet replicas to manage). JVM heap set proportionally in fe.conf (-Xmx2g)."
}
