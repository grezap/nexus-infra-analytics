...
- **starrocks-sn** (`analytics-starrocks.ps1 cycle`) — **DONE 2026-06-26**: certs new-root + data plane proven (schema-bootstrap EXIT GATE GREEN: 60 rows, 3 BE, sharded+replicated). The prior session's apply was **interrupted by a build-host standby** mid-run; on resume the re-run apply finished the final two overlays (backup-repo — NFS mounted on all 6 nodes + `nexus_backups` filesystem repository registered, the broker-less `file://` snapshot remaining the documented ratification follow-up; operator-user — `nexus-cluster-admin` EXIT GATE GREEN, operator-pw reconciled from KV in-graph) and `smoke-0.G.6.ps1` re-ran **ALL PASSED**. Cluster graceful-stopped → base.
- **Fixed a latent hang in `role-overlay-starrocks-schema-bootstrap.tf`** surfaced by the resume: a redundant second SSH after the (already-green) in-script gate ran `mysql … -u root -p$(sudo cat /dev/null) …`, which expands to a bare `-p` → `mysql` blocked on an interactive password prompt over the no-tty SSH (hangs indefinitely once root is password-protected — this is what the prior session's apply actually froze on under standby). The probe's output was discarded (tablet distribution is already verified in-script via `SHOW BACKENDS` in steps 4a/4b), so it was removed.
