...
### §3.1 Cold-rebuild canon (overlay-apply path LIVE-RATIFIED 2026-05-23; full destroy→build→apply→smoke operator-gated)
...
```
Ratification checklist (flip to fully PROVEN when all checked):
- [x] `packer build` of both templates clean (keeper + server built live 2026-05-23)
- [x] `apply` ALL overlays green (server-config + schema-bootstrap + backup-repo + cross-env foundation/security)
- [x] `smoke-0.G.5.ps1` ALL GREEN (129/129) on the live cluster
- [ ] `terraform destroy` clean (no orphaned VMs / dirs) — full cold-rebuild not yet exercised
- [ ] from-zero rebuild (destroy → build → apply → smoke) with no operator hot-state — operator-gated
